...
const WorkspaceManagement = () => {
  ...
  // Verify owner access
  useEffect(() => {
    // Only check after user data is loaded (username will be populated)
    if (login.username && !login.isOwner) {
      router.push("/")
    }
  }, [login.isOwner, login.username, router])
...
// This page should not use any layout
WorkspaceManagement.layout = ({ children }: { children: React.ReactNode }) => <>{children}</>
